...
Class WordPress_Readme_Parser {
	...
	function parse_readme_contents( $file_contents ) {
		...
		if ( preg_match('|Last update : (.*)|i', $file_contents, $_last_update) )
			$last_update = $this->sanitize_text($_last_update[1]);
		else
			$last_update = NULL;

		if ( preg_match('|Last commiter : (.*)|i', $file_contents, $_last_commiter) )
			$last_commiter = $this->sanitize_text($_last_commiter[1]);
		else
			$last_commiter = NULL;
